/** 名言マスターDB — X自動投稿用（telegram-bot/quotes.tsと同一データ） */
...
export interface QuoteMaster {
  id: string;
  text: string;
  source: string;
  character?: string;
  category: QuoteCategory;
  type: "anime" | "manga" | "song" | "movie";
}

export const QUOTES_MASTER: QuoteMaster[] = [
  // ===== 無価値感 =====
...
export function getRandomQuote(excludeIds: string[] = []): QuoteMaster | null {
  const available = QUOTES_MASTER.filter((q) => !excludeIds.includes(q.id));
