...
async function processFile(
  ...
  // 既存データを削除
  await supabase.from("michelle_knowledge_parents").delete().eq("source", relativeSource);
  ...
  for (const parent of parents) {
    ...
    const { error: childError } = await supabase.from("michelle_knowledge_children").insert(childRows);
