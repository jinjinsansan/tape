...
import { submitLessonQuiz, CourseNotFoundError, LessonAccessError } from "@/server/services/courses";
...
export async function POST(request: Request, context: { params: { slug: string; lessonId: string } }) {
  ...
  let userId: string;
  try {
    const user = await getRouteUser(supabase, `Course quiz: ${slug}`);
    if (!user) {
  ...
  try {
    const course = await submitLessonQuiz(slug, lessonId, userId, parsed.data.answers);
