    ...
    >
      <div className="flex items-center justify-between border-b border-gray-300 bg-gray-100 px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <span className="rounded-full bg-gray-200 px-2 py-0.5 text-xs text-gray-700">{logs.length} logs</span>
          {logs.filter((l) => l.level === "error").length > 0 && (
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {logs.length === 0 ? (
          <div className="flex h-full items-center justify-center text-sm text-gray-500">No logs yet</div>
        ) : (
          <div className="space-y-1">
            {logs.map((log) => (
              <div
                key={log.id}
                className={cn("rounded border p-2 text-xs font-mono", LOG_COLORS[log.level])}
              >
                ...
                {log.data ? (
                  <details className="mt-1">
                    <summary className="cursor-pointer text-[10px] opacity-70">Show data</summary>
